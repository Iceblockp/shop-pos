...
export const DatabaseProvider: React.FC<{ children: ReactNode }> = ({
  ...
  useEffect(() => {
    const initDB = async () => {
      try {
        const database = await openDB<MiniMarketDB>("mini-market-pos", 2, {
          upgrade(db) {
            ...
            if (!db.objectStoreNames.contains("transactionItems")) {
              const transactionItemsStore = db.createObjectStore(
                "transactionItems",
                { keyPath: "id", autoIncrement: true }
              );
              transactionItemsStore.createIndex(
                "by-transaction",
                "transactionId"
              );
            }
  ...
  const seedInitialData = async (database: IDBPDatabase<MiniMarketDB>) => {
    ...
    const settingCount = await database.count("setting");
    if (settingCount === 0) {
      ...
      const tx = database.transaction("setting", "readwrite");
      await tx.store.add(defaultSetting);
      await tx.done;
    }
        ...
        const defaultProducts: Omit<Product, "id">[] = [
          {
            ...
            imageUrl:
              "https://images.pexels.com/photos/2995299/pexels-photo-2995299.jpeg?auto=compress&cs=tinysrgb&w=200",
            createdAt: new Date(),
            updatedAt: new Date(),
          },
          {
            ...
            imageUrl:
              "https://images.pexels.com/photos/5945755/pexels-photo-5945755.jpeg?auto=compress&cs=tinysrgb&w=200",
            createdAt: new Date(),
            updatedAt: new Date(),
          },
          {
            ...
            supplierId: suppliers[0].id!,
            imageUrl:
              "https://images.pexels.com/photos/2480468/pexels-photo-2480468.jpeg?auto=compress&cs=tinysrgb&w=200",
            createdAt: new Date(),
            updatedAt: new Date(),
          },
        ];
